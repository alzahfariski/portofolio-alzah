    ...
    return (
        <div className="flex flex-col gap-8 mt-4">
            {experiences.map((exp, index) => (
                <div key={index} className="flex flex-col gap-3 relative pl-6 border-l border-neutral-200 ">
                    {/* Dot Indikator Timeline */}
                    <div className="absolute -left-[5.5px] top-1.5 w-2.5 h-2.5 rounded-full bg-neutral-300  border-2 border-white " />
                    ...
                    <div className="flex flex-row items-center gap-2">
                        {exp.imageUrl && (
                            <img
                                ...
                                alt={exp.company}
                                className="w-6 h-6 object-contain rounded-md bg-white  p-0.5 border border-neutral-100 "
                            />
                        )}
                        <h3 className="text-sm font-bold text-neutral-800 ">
                            {exp.company}
                        </h3>
                        <span className="text-[10px] font-medium text-neutral-500 bg-neutral-100  px-2 py-0.5 rounded-full uppercase tracking-wider">
                            {exp.type}
                        </span>
                        <span className="text-[10px] font-medium text-neutral-500 bg-neutral-100  px-2 py-0.5 rounded-full uppercase tracking-wider">
                        ...
                        {[...exp.work].reverse().map((role, rIndex) => (
                            <div key={rIndex} className="flex flex-col gap-1.5">
                                <div className="flex justify-between items-start gap-4">
                                    ...
                                    <span className="text-[11px] font-medium text-neutral-500  whitespace-nowrap bg-neutral-50 px-2 py-0.5 rounded border border-neutral-100 ">
                                ...
                                {role.description && (
                                    <ul className="flex flex-col gap-2 mt-1"> {/* gap-2 mengatur jarak antar poin */}
                                        {role.description.split('•').filter(item => item.trim() !== '').map((point, pIndex) => (
                                            <li key={pIndex} className="text-xs text-neutral-600  leading-relaxed flex gap-2">
